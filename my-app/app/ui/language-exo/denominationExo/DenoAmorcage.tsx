...
interface Item {
    ...
    emoji: string;
    motCible: string;
    amorcage: string;       // indice phonologique (ex. "ch...")
    niveau: Niveau;
...
// Facile    : amorçage = 1er phonème, distracteurs de catégories très différentes
// Moyen     : amorçage = 1re syllabe, 1-2 distracteurs partagent l'attaque
// Difficile : amorçage = 1re syllabe, TOUS les choix partagent la même attaque
const ITEMS: Item[] = [
    // === FACILE ===
    { id:  1, emoji: "🍎", motCible: "pomme",    amorcage: "p...",   niveau: "facile",
      distracteurs: ["banane",    "soleil",   "maison"]   },
    { id:  2, emoji: "🐶", motCible: "chien",    amorcage: "ch...",  niveau: "facile",
      distracteurs: ["maison",    "table",    "orange"]   },
    { id:  3, emoji: "🚗", motCible: "voiture",  amorcage: "v...",   niveau: "facile",
      distracteurs: ["pomme",     "chat",     "livre"]    },
    { id:  4, emoji: "🌳", motCible: "arbre",    amorcage: "a...",   niveau: "facile",
      distracteurs: ["voiture",   "chien",    "table"]    },
    { id:  5, emoji: "🔑", motCible: "clé",      amorcage: "cl...",  niveau: "facile",
      distracteurs: ["pomme",     "maison",   "chien"]    },
    // === MOYEN (1-2 distracteurs partagent l'attaque) ===
    { id:  6, emoji: "🦋", motCible: "papillon", amorcage: "pa...",  niveau: "moyen",
      distracteurs: ["paravent",  "maison",   "chien"]    },
    { id:  7, emoji: "🍌", motCible: "banane",   amorcage: "ba...",  niveau: "moyen",
      distracteurs: ["bateau",    "orange",   "table"]    },
    { id:  8, emoji: "🐢", motCible: "tortue",   amorcage: "tor...", niveau: "moyen",
      distracteurs: ["torchon",   "cheval",   "livre"]    },
    { id:  9, emoji: "🌂", motCible: "parapluie",amorcage: "pa...",  niveau: "moyen",
      distracteurs: ["parasol",   "voiture",  "chat"]     },
    { id: 10, emoji: "🏠", motCible: "maison",   amorcage: "mai...", niveau: "moyen",
      distracteurs: ["main",      "chien",    "voiture"]  },
    // === DIFFICILE (tous les choix partagent la même attaque) ===
    { id: 11, emoji: "🐸", motCible: "grenouille",amorcage:"gre...", niveau: "difficile",
      distracteurs: ["grenade",   "grenier",  "grêle"]    },
    { id: 12, emoji: "🌹", motCible: "rose",     amorcage: "ro...",  niveau: "difficile",
      distracteurs: ["robe",      "roue",     "robinet"]  },
    { id: 13, emoji: "🎵", motCible: "musique",  amorcage: "mu...",  niveau: "difficile",
      distracteurs: ["mulet",     "mur",      "muscle"]   },
    { id: 14, emoji: "🦊", motCible: "renard",   amorcage: "re...",  niveau: "difficile",
      distracteurs: ["renfort",   "renne",    "rempart"]  },
    { id: 15, emoji: "🌙", motCible: "lune",     amorcage: "lu...",  niveau: "difficile",
      distracteurs: ["lutin",     "lunette",  "lustre"]   },
];
...
export default function DenoAmorcage({ patientId }: { patientId: string | null }) {
    ...
    const handleSelect = (mot: string) => {
        ...
        if (currentIndex + 1 >= items.length && patientId) {
            ...
            saveScore({
                patientId,
                exercice: "Dénomination avec amorçage",
                domaine: "language",
                score,
                empan,
            }).catch(console.error);
            ...
            {phase === "idle" && (
                <div className="text-center space-y-6 max-w-md">
                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
                        Une image s'affiche avec le <strong>début du mot</strong> comme indice.
                        Choisissez le mot correspondant parmi les 4 propositions.<br />
                        La difficulté augmente : au niveau difficile, tous les choix
                        commencent par la même syllabe.
                    </p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">
                        {ITEMS.length} images · amorçage phonologique progressif
                    </p>
                    <button
                        onClick={startGame}
            ...
            {(phase === "playing" || phase === "feedback") && currentItem && (
                <div className="flex flex-col items-center gap-5 w-full max-w-sm">
                    ...
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-50 dark:bg-teal-900/30 border border-teal-200 dark:border-teal-700">
                            <span className="text-xs text-teal-500 dark:text-teal-400 font-medium uppercase tracking-wide">
                                indice
                            </span>
                            <span className="text-lg font-bold text-teal-700 dark:text-teal-300 tracking-widest">
                                {currentItem.amorcage}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800 rounded-lg px-4 py-3 text-xs space-y-2 text-left max-h-52 overflow-y-auto">
                        {results.map((r, i) => (
                            <div key={i} className="flex items-center gap-2">
                                ...
                                <span className="shrink-0 font-mono text-teal-600 dark:text-teal-400 text-xs">
                                    {r.item.amorcage}
                                </span>
