...
const DIFFICULTY_CONFIG: Record<Difficulty, {
    cols: number;
    rows: number;
    targetRatio: number;   // proportion totale des deux cibles
    timeSec: number;
    cellPx: number;
    fontSize: string;
    empan: number;
    targetPairs: Array<[string, string]>; // paires de cibles possibles
    distractorBoost: string[];
}> = {
    facile: {
        cols: 8, rows: 10, targetRatio: 0.28, timeSec: 90,
        cellPx: 42, fontSize: "text-xl", empan: 3,
        targetPairs: [["A", "E"], ["S", "T"], ["R", "N"], ["I", "L"]],
        distractorBoost: [],
    },
    moyen: {
        cols: 10, rows: 12, targetRatio: 0.22, timeSec: 60,
        cellPx: 34, fontSize: "text-base", empan: 5,
        targetPairs: [["B", "D"], ["P", "F"], ["H", "M"], ["G", "C"]],
        distractorBoost: ["B", "D", "P", "R", "F", "E", "H"],
    },
    difficile: {
        cols: 12, rows: 14, targetRatio: 0.18, timeSec: 45,
        cellPx: 28, fontSize: "text-sm", empan: 7,
        targetPairs: [["C", "G"], ["O", "Q"], ["U", "V"], ["I", "J"]],
        distractorBoost: ["C", "G", "O", "Q", "U", "D", "J", "V", "I"],
    },
};
...
function buildGrid(diff: Difficulty): { grid: GridItem[]; target1: string; target2: string } {
    const cfg = DIFFICULTY_CONFIG[diff];
    const total = cfg.cols * cfg.rows;
    const totalTargetCount = Math.round(total * cfg.targetRatio);
    const countPerTarget   = Math.floor(totalTargetCount / 2);

    // Pick a random pair
    const pair = cfg.targetPairs[Math.floor(Math.random() * cfg.targetPairs.length)];
    const [target1, target2] = pair;

    // Build distractor pool (all letters except both targets)
    const base = LETTERS.filter(l => l !== target1 && l !== target2);
    const pool: string[] = [...base];
    cfg.distractorBoost
        .filter(c => c !== target1 && c !== target2)
        .forEach(c => pool.push(c, c)); // 3× weight for visually similar

    const items: Array<{ char: string; isTarget: boolean }> = [];
    for (let i = 0; i < countPerTarget; i++)   items.push({ char: target1, isTarget: true });
    for (let i = 0; i < countPerTarget; i++)   items.push({ char: target2, isTarget: true });
    const remaining = total - countPerTarget * 2;
...
export default function BarrageDouble({ patientId }: { patientId: string | null }) {
    ...
    const startGame = useCallback((diff: Difficulty) => {
        const { grid: g, target1: t1, target2: t2 } = buildGrid(diff);
        setGrid(g);
        setTarget1(t1);
        setTarget2(t2);
            ...
            {phase === "idle" && (
                <div className="flex flex-col items-center justify-center flex-1 gap-6 p-6 text-center">
                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed max-w-md">
                        Une grille de lettres s'affiche. <strong>Deux lettres cibles</strong> sont indiquées en haut.<br />
                        Cliquez le plus vite possible sur <strong>toutes les occurrences</strong> de l'une <em>ou</em> de l'autre
                ...
                <>
                    ...
                    <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0 gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-400 dark:text-slate-500 font-bold">et</span>
                            <span className="text-3xl font-bold font-mono text-violet-600 dark:text-violet-400 w-10 text-center">
                                {target2}
                            </span>
                        </div>
                ...
                <div className="flex flex-col h-full overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0 flex-wrap gap-2">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-slate-400 font-bold">et</span>
