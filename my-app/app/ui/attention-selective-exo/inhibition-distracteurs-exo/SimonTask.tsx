...
interface Trial {
    ...
    side:    Side;
    type:    TrialType;
    correct: Response; // réponse attendue (basée sur la couleur)
}
...
// ─── Règle couleur → réponse ──────────────────────────────────────────────────

const COLOR_RESPONSE: Record<Color, Response> = {
    rouge: "left",
    bleu:  "right",
};
...
function buildTrials(diff: Difficulty): Trial[] {
    ...
    // Congruent : la position correspond à la réponse correcte
    // Rouge → Gauche → cercle à gauche  |  Bleu → Droite → cercle à droite
    for (let i = 0; i < mix.congruent; i++) {
        const color: Color    = Math.random() < 0.5 ? "rouge" : "bleu";
        const correct: Response = COLOR_RESPONSE[color];
        const side: Side      = correct === "left" ? "left" : "right";
        trials.push({ color, side, type: "congruent", correct });
    }

    // Neutre : cercle au centre (pas de conflit spatial)
    for (let i = 0; i < mix.neutral; i++) {
        const color: Color    = Math.random() < 0.5 ? "rouge" : "bleu";
        const correct: Response = COLOR_RESPONSE[color];
        trials.push({ color, side: "center", type: "neutral", correct });
    }

    // Incongruent : la position est OPPOSÉE à la réponse correcte
    for (let i = 0; i < mix.incongruent; i++) {
        const color: Color    = Math.random() < 0.5 ? "rouge" : "bleu";
        const correct: Response = COLOR_RESPONSE[color];
        const side: Side      = correct === "left" ? "right" : "left";
        trials.push({ color, side, type: "incongruent", correct });
    }
...
const circleColor: Record<Color, string> = {
    rouge: "bg-red-500 shadow-red-300 dark:shadow-red-900",
    bleu:  "bg-blue-500 shadow-blue-300 dark:shadow-blue-900",
};
...
export default function SimonTask({ patientId }: { patientId: string | null }) {
    ...
    // Refs — lecture stable depuis les timers
    ...
    const handleResponse = useCallback((resp: Response) => {
        ...
        const trial   = trialsRef.current[currentIdxRef.current];
        const correct = resp === trial.correct;
        const result: TrialResult = { trial, response: resp, correct, rt };
        resultsRef.current = [...resultsRef.current, result];
        setRenderRes([...resultsRef.current]);
    ...
    function btnCls(resp: Response): string {
        ...
        const responded = renderRes.at(-1)?.response === resp;
        ...
        if (!responded && currentTrial && resp === currentTrial.correct && (feedback === "wrong" || feedback === "timeout"))
    ...
    // ── Header de progression (réutilisé) ─────────────────────────────────────
        ...
        return (
            <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                <div className="flex items-center gap-3">
                    <span className="text-xs text-slate-400 shrink-0">{done} / {totalTrials}</span>
            ...
            {phase === "idle" && (
                <div className="flex flex-col items-center justify-center flex-1 gap-6 p-6 text-center">
                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed max-w-md">
                        Un cercle coloré apparaît à <strong>gauche</strong>, au <strong>centre</strong> ou à <strong>droite</strong>.<br />
                        Répondez selon sa <strong>couleur</strong> — ignorez sa position&nbsp;:
                    </p>
                    ...
                    <div className="flex gap-6 items-center">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">ROUGE → ← Gauche</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full bg-blue-500 shadow-md" />
                            <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">BLEU → Droite →</span>
                        </div>
                    </div>
            ...
            {phase === "stimulus" && currentTrial && (
                <>
                    ...
                    <div className="flex flex-col items-center justify-center flex-1 gap-6 p-4">
                        ...
                        {/* Légende rappel */}
                        <div className="flex gap-5 items-center text-xs text-slate-400 dark:text-slate-500 select-none">
                        ...
                        {/* Zone stimulus — 3 colonnes : gauche | centre | droite */}
                        <div className="relative w-full max-w-sm h-28 flex items-center select-none">
                            {/* Séparateurs visuels */}
                            <div className="absolute inset-0 flex pointer-events-none">
                                <div className="flex-1 border-r border-dashed border-slate-200 dark:border-slate-700" />
                                <div className="flex-1 border-r border-dashed border-slate-200 dark:border-slate-700" />
                                <div className="flex-1" />
                            </div>

                            {/* Cercle coloré */}
                            <div className={`absolute w-20 h-20 rounded-full shadow-xl transition-none ${circleColor[currentTrial.color]} ${
                                currentTrial.side === "left"
                                    ? "left-[16.67%] -translate-x-1/2"
                                    : currentTrial.side === "center"
                ...
                <div className="flex flex-col items-center justify-center flex-1 gap-6 p-6 text-center">
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-center max-w-sm">
                        {renderRes.map((r, i) => (
                            <div
                                key={i}
                                title={`Essai ${i + 1} · ${r.trial.type} · ${r.trial.color} · ${r.correct ? "✓" : "✗"} · ${r.rt > 0 ? r.rt + "ms" : "timeout"}`}
